...
resource "aws_eip" "nat_public_ip_2" {
  ...
  depends_on = [aws_internet_gateway.internet_gateway]
  tags = merge({
    Name = "${var.deployment_name}-${var.vpc_name}-nat-eip-2"
  }, local.common_tags)
}
...
resource "aws_nat_gateway" "nat_gateway_2" {
  ...
  tags = merge({
    Name = "${var.deployment_name}-${var.vpc_name}-nat-2"
  }, local.common_tags)
}
...
resource "aws_route_table" "private_route_table_2" {
  vpc_id = aws_vpc.vpc.id
  tags = merge({
    Name = "${var.deployment_name}-${var.vpc_name}-private-rt-2"
  }, local.common_tags)
}
...
# Update private subnet associations: 1 uses NAT 1, 2 uses NAT 2, 3 can use either (or round-robin)
